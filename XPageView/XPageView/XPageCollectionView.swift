...
extension XPageCollectionView {
    fileprivate func setupSubViews() {
        /// titleView
        let titleH = titleStyle.isShowBotomLine ? titleStyle.titleHeight + titleStyle.bottomLineHeight : titleStyle.titleHeight
        let titleY = isTitleInTop ? 0 : (bounds.height - titleH)
        let titleFrame = CGRect(x: 0, y: titleY, width: kScreenWidth, height: titleH)
        titleView = XTitleView(frame: titleFrame, titles: titles, style: titleStyle)
